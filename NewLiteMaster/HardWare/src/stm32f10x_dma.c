...
int DMA_GetChanleStatu(DMA_Channel_TypeDef* DMAy_Channelx,FunctionalState NewState)
{
	 /* Check the parameters */
  assert_param(IS_DMA_ALL_PERIPH(DMAy_Channelx));
  assert_param(IS_FUNCTIONAL_STATE(NewState));
  ...
  {
    /* Enable the selected DMAy Channelx */
    return ((DMAy_Channelx->CCR & CCR_ENABLE_Set)==0);
  }
  else
  {
    /* Disable the selected DMAy Channelx */
    return ((DMAy_Channelx->CCR & CCR_ENABLE_Set)==CCR_ENABLE_Set);
  }
}
